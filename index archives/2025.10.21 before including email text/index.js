...
async function shopifyFilesCreateFromStaged(stagedTargets) {
  const filesInput = stagedTargets.map(t => ({ originalSource: t.resourceUrl }));
...
/** Merge and write NEW + EXISTING to the list.file_reference metafield */
async function addFilesToOrderReferenceImages(order, newFileIds) {
  ...
  const merged = Array.from(new Set([...existingIds, ...newFileIds])); // de-dupe
  ...
  const mfInput = {
    ...
    // IMPORTANT: For list.* reference-types we set the *value* to a JSON string of IDs
    value: JSON.stringify(merged)
  };

  // Retry a bit to ride out any Files propagation delay
  const MAX_TRIES = 5;
  const DELAY_MS  = 1500;
  let lastErr = null;
  ...
  throw lastErr || new Error('metafieldsSet(list.file_reference) failed after retries');
}

/** End-to-end: pull image attachments from the latest inbound Gmail message and attach */
async function uploadEmailImagesToOrderMetafield({ latestMessage, order }) {
  ...
  await sleep(1500); // small settle time
  ...
  try {
    ...
    if (!customerEmail) {
      await client.chat.postMessage({
        ...
        text: '❌ Could not find a *customer* email in the forwarded content. Please reply with the correct address.'
      });
      return;
    }
    ...
    if (!order?.name) {
      await client.chat.postMessage({
        channel: event.channel,
        thread_ts: event.thread_ts || event.ts,
        text: `❌ No Shopify order with tag *NeedPhotoNoShip* found for ${customerEmail}.`
      });
      return;
    }
...
// --- Attach any images from the customer email thread to custom.reference_images ---
try {
  let subjectGuess = '';
  try {
    const payload = JSON.parse(body.actions?.[0]?.value || '{}');
  ...
  if (foundThread) {
    ...
    const result = await uploadEmailImagesToOrderMetafield({ latestMessage: latestInbound, order });
  ...
  logger?.error?.('reference_images attach failed (App 2)', attachErr);
